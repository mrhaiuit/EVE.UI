...
(function($) {
	...
	$.fn.tinymce = function(settings) {
		...
		if (!win.tinymce && !lazyLoading && (url = settings.script_url)) {
			...
			// make sure that all plugins_GiaoDien2, themes etc are loaded in source mode as well
			...
			if (url.indexOf('gzip') != -1) {
				...
				url = url + (/\?/.test(url) ? '&' : '?') + "js=true&core=true&suffix=" + escape(suffix) +
					"&themes=" + escape(settings.theme || 'modern') + "&plugins_GiaoDien2=" +
					escape(settings.plugins_GiaoDien2 || '') + "&languages=" + (lang || '');
				...
				if (!win.tinyMCE_GZ) {
					win.tinyMCE_GZ = {
						start: function() {
							...
							// Add plugins_GiaoDien2 with languages
							$.each(settings.plugins_GiaoDien2.split(","), function(i, name) {
								if (name) {
									load("plugins_GiaoDien2/" + name + "/plugin" + suffix + ".js");
									load("plugins_GiaoDien2/" + name + "/langs/" + lang + ".js");
								}
							});
						},
